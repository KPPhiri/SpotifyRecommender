...
spotifyApi.setAccessToken('BQBWYryJyXpOLxFF4Q3HYA03bizOw4G0iAJ9DdOO1HdQ2fEEUnbai8WWnARyEbAoklrPJUwpN3NebZAYMYV0Ey9i8jg0TwfJgI9HadwECdP2PRlaqcxZot7uk9JwkG5YCha3vjbGL3GEYhxlu8wsnDMPyZe23gd1dVmJFojjyyO_0CGydUgaFNYzgDZwcswkFXTXjfLQWKYDPv8');
...
function getTrackFeatures(playlist_ids_arr){
	spotifyApi.getAudioFeaturesForTracks(playlist_ids_arr)
	  .then(function(data) {
	    ...
	    for(let track_indx = 0; track_indx < items.length; track_indx++) {
	  	...
	  	if(done){
			runTensor(features, Array.from(Array(labels.length).keys()), k);
      return [features, labels, tracks_name]
	  	}
